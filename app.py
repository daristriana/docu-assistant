...
# As requested, we'll use these URLs for the prototype.
# WebBaseLoader can take a list of URLs to scrape.
DOCS_URLS = [
...
def load_and_index_docs(urls, api_key):
    """
    ...
    try:
        # 1. Load Documents
        # WebBaseLoader fetches and parses HTML from the URLs
        loader = WebBaseLoader(urls)
        docs = loader.load()
        
        # 2. Split Documents
        # RecursiveCharacterTextSplitter is good for generic text
        ...
        # This creates the vector store in memory from the split documents
        vector_store = FAISS.from_documents(split_docs, embeddings)
        
        # 5. Create Retriever
        # This object is used to fetch relevant documents
...
def get_contextual_retriever_chain(retriever, llm):
    ...
    # This prompt helps the LLM rephrase the user's question based on history
...
def get_conversational_rag_chain(retriever_chain, llm):
    ...
    # This is the core prompt. It *must* instruct the LLM to follow our rules.
    system_prompt = f"""
...
def get_escalation_summary(chat_history_messages, user_question, failed_solution, llm):
    ...
    # Format the history for the summary prompt
    history_str = "\n".join(
...
with st.sidebar:
    ...
    if "GOOGLE_API_KEY" in st.secrets:
        ...
        # Fallback to text input if not in secrets (for local testing)
...
# 3. Initialize LLM
# We use a model that's good for chat (RAG)
...
retriever = load_and_index_docs(DOCS_URLS, google_api_key)
...
if user_prompt:
    ...
    # Check if the user is reporting a failed solution
    ...
    if is_failure_report and len(st.session_state.messages) > 2:
        ...
        with st.chat_message("ai"):
            with st.spinner("I'm sorry to hear that. Generating a summary for support..."):
                ...
                # 2. Create the response for the user
